...
#define	eeLIMIT_BAT_FAULT_OFFSET		34
#define	ee_GAIN							36
#define	ee_AMP_NO						37
...
typedef union {
	 ...
	 struct  {
		...
		u8	fToneSelect		: 1;	/* 0 - Continuous Horn Pattern, 1 - Temporal Horn Pattern - for future implementation*/
		u8	fEndOfLife		: 1;	/* End of Life Enable bit - for future implementation*/
		u8	fLowBatHush		: 1;	/* Low Battery Hush Enable bit (to 8 hour) - for future implementation */
		u8	fDepassiv		: 1;	/* Depassivation Low Impendans Battery */
	 };
}tCfgReg;
...
typedef struct  {
	...
	u16			limit_chamber_low;	// Нижний порог определения неисправности камеры (increment)
	u16			limit_chamber_hi;	// Верхний порог определения неисправности камеры (increment)
	u16			limit_bat;			// Порог определения неисправности батареи (в 10мВ)
